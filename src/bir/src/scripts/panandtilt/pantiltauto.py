...
class motor_movement():
    ...
    def __init__(self):
        ...
        self.pan_sub = rospy.Subscriber("/pan_controller/state", JointState, self.pan_callback)
        self.tilt_sub = rospy.Subscriber("/tilt_controller/state", JointState, self.tilt_callback)
...
class image_capture():
    ...
    def save_image(self, x, y):
        cv2.imwrite('/home/fredlaranja/Pictures/image_' + str(x) + str(y) + '.png', self.cv_image)
